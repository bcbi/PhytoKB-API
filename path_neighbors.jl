...
function get_names(leaf_arr, dict_path, length_dict, dname_dict)
    leaf_nam = Any[]
    for i=1:length(leaf_arr)
...
function get_neighbors(input_txn, input_lvl, path_dict)
    input_txn_path = path_dict[input_txn]
    input_txn_path = split(input_txn_path, "\.")
    #println(input_txn_path[1:end-(2*input_lvl)])
    #println(input_txn_path)
    input_txn_parent = join(input_txn_path[1:end-(2*input_lvl-1)], "\.")
...
function create_newick(nbr_arr, qry_arr)
...
nwk_str = create_newick(nbr_arr, qry_arr)
println(nwk_str)
